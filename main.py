...
# Define symptoms dictionary & disease list (as in your original code)
...
# ✅ Updated Route for Web & API
@app.route('/predict', methods=['POST'])
def predict():
    symptoms = request.form.get('symptoms') or request.json.get('symptoms')  # Handle both form and JSON input

    if not symptoms:
        error_message = "No symptoms provided. Please enter valid symptoms."
    ...
    # If the request is from an API client (expects JSON)
    if request.accept_mimetypes.accept_json:
        return jsonify({
    ...
    # If request is from a browser, render HTML
...
if __name__ == '__main__':
    port = int(os.environ.get("PORT", 5000))  # Handle Render's port
